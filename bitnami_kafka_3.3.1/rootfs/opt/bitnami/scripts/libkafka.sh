...
kafka_create_sasl_scram_zookeeper_users() {
    ...
    for ((i = 0; i < ${#users[@]}; i++)); do
        ...
        # Ref: http://docs.confluent.io/current/kafka/authentication_sasl/authentication_sasl_scram.html#sasl-scram-overview
        if [[ "${KAFKA_ZOOKEEPER_PROTOCOL:-}" =~ SSL ]]; then
...
kafka_configure_internal_communications() {
    ...
    if [[ "${allowed_protocols[*]}" =~ $protocol ]]; then
        ...
        if [[ "$protocol" = "SASL_PLAINTEXT" ]] || [[ "$protocol" = "SASL_SSL" ]]; then
            ...
            # For more information, see: http://docs.confluent.io/current/kafka/authentication_sasl/authentication_sasl_plain.html#sasl-plain-overview)
